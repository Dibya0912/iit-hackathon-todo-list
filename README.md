...
The shared leaderboard lists every registered adventurer and ranks the current Monday-to-Monday UTC week by verified leaderboard XP. A quest completed less than five minutes after creation scores zero leaderboard XP. From five minutes onward it earns its difficulty XP plus 5 time XP per completed five-minute block, capped at one additional difficulty reward (Easy max 20, Medium max 50, Hard max 100). Character XP and Gold still follow the chosen difficulty. Rankings refresh every 15 seconds. After the week closes, qualifying places receive one server-side award: #1 gets 1000 XP, #2 gets 500 XP, #3 gets 200 XP, and #4–#10 get 100 XP each. Zero-score accounts remain visible but do not receive a prize. Higher leaderboard XP immediately moves a player ahead; equal scores use the older account as a stable tie-break. Creation time, completion time, elapsed seconds, and awarded leaderboard XP are stored server-side. Awards are recorded once in `leaderboard_rewards` and the economy ledger.
